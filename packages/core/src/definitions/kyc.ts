...
// KYC URL builder — requires apiUrl to be passed at runtime
export function buildKycUrl(apiUrl: string) {
  const v2 = apiUrl.replace(/\/v1\/?$/, '/v2');
  return {
    setName: 'user/name',
    setData: 'user/data',
    file: 'kyc/file',
    ...
    checkTfa: 'kyc/2fa',
    limit: `${v2}/kyc/limit`,
    transfer: (client: string) => `${v2}/kyc/transfer?client=${encodeURIComponent(client)}`,
  };
}
